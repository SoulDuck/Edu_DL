...
ada_lr = 0.01
mmt_lr = 0.00005
momentum(start_x, start_y, mmt_lr, tf_beale, mmt)
adagard(start_x, start_y, ada_lr, tf_beale)

# Session
sess = tf.Session()
sess.run(tf.global_variables_initializer())
mm_xs, mm_ys, mmz_zs = trace(sess, 50000, 'Momentum')
ada_xs, ada_ys, ada_zs = trace(sess, 50000, 'Adagrad')

# Visualization
ax = generate_beale()

# add initial point [start_x] ,[start_y], [start_z]
ax.plot([start_x]+mm_xs, [start_y]+mm_ys, [start_z]+mmz_zs, label='momentum(beta={}, lr={})'.format(mmt,mmt_lr), color='b')
ax.plot([start_x]+ada_xs, [start_y]+ada_ys, [start_z]+ada_zs, label='adagrad(lr ={})'.format(ada_lr), color='g')
